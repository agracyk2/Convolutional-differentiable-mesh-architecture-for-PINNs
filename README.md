...
In this repository, our code learns the mapping from initial data of CIFAR-10. This can be changed quite flexibily. A more typical example of when to use this architecture, since it is suitable for PINN-type algorithms, is to map an initial condition $\psi$ to the solution over a grid, where we train the PINN using an output $\tilde{\psi}_{t=0} of the same initial condition $\psi$ at $t=0$.
